...
これを受けて一様な利得 0.3 と 0.4 を追加した。0.3 は総量 +24、速度 +2.5 で 40 ステップで回復(安全域の上限)。0.4 は 0.5 と同じく飽和した。**一様な刺激で目に見える応答を得られる安全域は 0.2〜0.3 で、それでも「太る」以上の変化にはならない。** 形や向きの変化が欲しいなら、空間的に非対称な刺激(勾配など)が要る。

![一様 0.3](02_report/long_growth_uniform_g0.3.png)

**実際の曲(約 5 分のインストゥルメンタル、MP3 → 自動変換、growth 0.2、勾配)での本人の観察:** 「少しずつ方向が変わる。速さは変わらない。基本は上から下への動きで、その向きが変化している」。数値と整合する。曲は最大音量で正規化するので通常の場面の振幅は 0.3〜0.6 程度、実効利得は 0.06〜0.12 で総量の変化は +5〜10% にとどまり、太さの変化は見えない。向きだけが累積して見える。勾配に対して垂直な向き(真下)に寄る傾向は、勾配 run のシートと本人の観察の両方で見られたが、原因(勾配の継ぎ目か、勾配そのものか)は未分離。
