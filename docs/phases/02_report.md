...
> **正誤表(2026-09-24、Phase 4 で発見):** この報告で「10 秒パルス」「継続」と書いた run は、実際には **2 秒**のパルスだった。合成パルス源に「`--pulse-count 1` でも既定の周期 2 秒でパルスが切れる」不具合があったため(修正はコミット bb39c7b、回帰テストあり)。WAV の run と 0.5 秒パルスの run は影響を受けない。グラフの帯と系列名は 2 秒に直した。本当に 10 秒当てた再実験の結果は末尾の「正誤表: 10 秒パルスの再実験」を参照。一様な刺激についての結論は変わらない。勾配については、線形勾配 0.2 を 10 秒当てると刺激後に消滅することが分かった。
...
## 正誤表: 10 秒パルスの再実験(コミット bb39c7b、`experiments/out/phase2_fix`)

本当に 10 秒(step 120〜720)当てた結果。無音の基準との差。

| run | 総量差 最大 | 向き差 最大 | 回復 | 終了 |
| --- | --- | --- | --- | --- |
| growth 一様 0.05 / 0.1 / 0.2 / 0.3 | +3.6 / +7.0 / +16.2 / +23.9 | ≤3° | 40 / 40 / 40 / 280 ステップ | 生存(2 秒のときと同じ高さで頭打ち) |
| growth 一様 0.5 | +782 | — | — | 飽和 |
| growth 勾配 0.05 / 0.1 | +2.5 / +5.9 | 7° / 36° | 20 / 戻らない | 生存 |
| growth 勾配 **0.2** | +14.7 | 35° | — | **刺激終了の 60 ステップ後に消滅**(2 秒では生存していた) |
| growth 勾配 0.3 | +22.7 | 32° | 戻らない | 生存(0.2 で死に 0.3 で生きる非単調は未解明) |
| growth 勾配 0.5 | +777 | — | — | 飽和 |
| mu 一様 0.005 / 0.01 | +1.1 / +3.3 | ≤2° | 20 / 40 | 生存 |
| mu 一様 ≥0.02、mu 勾配 ≥0.01 | — | — | — | 消滅(2 秒のときと同じ、最初の数十ステップで死ぬ) |

![勾配 0.2 を 10 秒](02_report/erratum_long_growth_gradient_x_g0.2_10s.png)

結論の変更点: 「勾配 0.1〜0.2 は見える変化、回復あり」→ **0.2 を 10 秒以上当てると消滅する。安全域は 0.1**。一様な刺激と mu の結論は変わらない。
