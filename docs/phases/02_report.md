...
成長関数への一様な加算(反応の強さ(コードでは `stim_gain`) 0.2)では、刺激が入った瞬間に総量が 71 → 87 に上がり、継続中は 84〜85 で保たれ、停止後 40〜60 ステップ(0.7〜1 秒)で 71 に戻った。速度も +1.5〜1.7 上がって戻る。進行方向は 1° 以内で変わらない。0.5 秒でも 10 秒でも最大偏差は同じ(応答が飽和する)。
...
### 3.3 反応の強さの境界: 効果なし / 見える変化 / 破綻

| 入り口 | 形 | 効果なし〜小 | 見える変化、回復あり | 破綻 |
| --- | --- | --- | --- | --- |
| growth | 一様 | 0.05(総量 +3.6) | 0.1〜0.3(+7〜+24、40〜60 ステップで回復) | ≥0.4: 個体が消え、格子全体が帯状の飽和模様になる(総量 800 超) |
...
### 3.3b 本人の実機観察(Mac、2026-09-24)

`cave_window --wav` を Mac で実行した本人の印象: **一様な刺激(kick120 / tone、growth 0.2)は違いがほとんど分からず、勾配(tone、gradient_x)は向きが変わって一目で分かった。** 数値と整合する。一様な刺激は総量 +16(約 20%、見た目は「少し太る」)で、キックは包絡が短いため +3 にとどまる。向きの変化は 20〜40° なので目立つ。

これを受けて一様な反応の強さ 0.3 と 0.4 を追加した。0.3 は総量 +24、速度 +2.5 で 40 ステップで回復(安全域の上限)。0.4 は 0.5 と同じく飽和した。**一様な刺激で目に見える応答を得られる安全域は 0.2〜0.3 で、それでも「太る」以上の変化にはならない。** 形や向きの変化が欲しいなら、空間的に非対称な刺激(勾配など)が要る。

![一様 0.3](02_report/long_growth_uniform_g0.3.png)

**実際の曲(約 5 分のインストゥルメンタル、MP3 → 自動変換、growth 0.2、勾配)での本人の観察:** 「少しずつ方向が変わる。速さは変わらない。基本は上から下への動きで、その向きが変化している」。数値と整合する。曲は最大音量で正規化するので通常の場面の振幅は 0.3〜0.6 程度、実効的な反応の強さは 0.06〜0.12 で総量の変化は +5〜10% にとどまり、太さの変化は見えない。向きだけが累積して見える。勾配に対して垂直な向き(真下)に寄る傾向は、勾配 run のシートと本人の観察の両方で見られたが、原因(勾配の継ぎ目か、勾配そのものか)は未分離。

### 3.4 WAV(生成音源、8 秒 + 無音 4 秒)

| run | 入り口 / 形 / 反応の強さ | 総量の最大偏差 | 向きの最大偏差 | 回復 |
| --- | --- | --- | --- | --- |
...
**再現性:** 振幅 0 の run は Phase 1 のコードとバイト単位で一致することをテストで保証(`test_stimulus`)。同じ入り口・形・反応の強さで短い/継続パルスの刺激中の最大偏差が一致した(応答が決定的で飽和する)。
...
- **mu の許容幅が狭い**(0.01 と 0.02 の間に生死の境界)。作品で mu を使うなら反応の強さを 0.01 以下に固定するか、Phase 4 の可塑性で感受性を下げる方向が現実的。
- **RMS の正規化はファイル単位。** リアルタイム入力(マイク)では最大値が事前に分からない。Phase 2 の範囲外だが、後で自動的な反応の強さの調整を考える。
...
1. **刺激の長期平均 m(t)(時定数 数秒〜数十秒)を持ち、それで反応の強さを下げる**(順応)。同じ音でも、直前まで大きな音が続いていれば応答が小さくなる。3.1 の「応答が飽和する」性質と組み合わせると、履歴による差が出やすい。
2. **m(t) で mu を少しだけずらす**(反応の強さ 0.005 以下の範囲)。長期的な音の多さで個体の「体質」が変わる。ただし mu の許容幅が狭いので、破綻の境界を先に測る必要がある。
3. **向きの持続は履歴の指標から除外する。** 位置と向きは中立安定なので、履歴の効果は総量・広がり・速度・応答の大きさで測る。

**判断してほしいこと:**

1. Phase 3 は上の 1(順応)から始めてよいか。
2. 作品の刺激として、入り口は growth(丈夫だが「太る」応答)と mu(繊細だが許容幅が狭い)のどちらを主にするか。両方残すか。実機観察では一様な刺激は目立たなかったので、**作品では空間的に非対称な形(勾配、または個体の位置に対する局所)を主にする**案が有力。
3. 勾配の形を周期境界に合うもの(cos)に変えるか、勾配自体を作品では使わないか。
4. Mac で `cave_window --wav` を試して、音と動きの同期、体感の応答(反応の強さ 0.1〜0.2)を確認してほしい。Suno の MP3 は `scripts/to_wav.sh` で変換できる。
5. RMS の正規化(ファイル内最大)で問題ないか。実際の曲では応答が小さくなることを本人が確認した。直近数秒の平均で正規化する案は Phase 3 の順応と同じ機構になる。
6. 色は入れていない(内部状態の値を写すだけ)。音で色を変えるなら「表示の層」として内部状態と分け、後のフェーズで判断する。
...
long_growth_uniform_g0.3                          23.70       0.62       1.635       2.542      -0.362       40       ok
long_growth_uniform_g0.4                         744.01     157.13      20.264      21.186     813.531        -  not-rec
