...
**Phase 4(可塑性)の候補。** 生き物には「つまみ」が 3 つある。g(利得)= 音への敏感さ、sigma = 丈夫さ(生きられる範囲の広さ)、mu = ちょうどいい密度。Phase 3 の慣れ(m)は一時的で、静かにしていれば数十秒で戻る。Phase 4 の可塑性は「戻らない変化」で、経験で性格が変わることに相当する。

1. **敏感さのつまみ g を、生き物に自分で回させる。** 音の多い環境では下がり、少ない環境では上がる(例: `g += η·(m − m_target)` を遅く回す)。慣れ(m)と性格(g)が二重の時定数になる。
2. **丈夫さのつまみ sigma も候補。** ただし、どこまで回すと死ぬかを先に測る(Orbium の生存域は狭い)。
3. **mu は回さない。** Phase 2 で少し動かしただけで消滅した。

確かめ方: 違う環境で育てた個体に、静かな時間の後で同じ音を聞かせ、反応の差が残っているかを見る。つまみがどこまで回ったかを記録する。
