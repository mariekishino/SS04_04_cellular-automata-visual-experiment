...
フェーズ文書 [03_history.md](03_history.md) の実装範囲 1〜6 のうち、**実曲での比較(4 の一部)は本人が Mac で実施中**で、この報告には含まれていない。

## 3. 観察結果

すべて Orbium、64×64、周期境界、時間基準 60 steps/s、入り口 growth、一様、利得 0.3。手順は [02_experiment_protocol.md](../02_experiment_protocol.md) の Phase 3 の定義どおり: 0〜20 s に履歴(H0 無音 / H1 まばら: 0.5 s を 4 s ごとに 5 回 / H2 密: 0.5 s を 1 s ごとに 19 回 / H3 連続: 18 s)、共通の無音(2 / 5 / 20 / 60 s)、その後に同じ探針(0.5 s、振幅 1)。応答 = 探針中の総量の最大値 − 直前の総量。
...
- **実曲での比較は未了。** 本人が Mac で `--wav 曲名 --wav-until 60 --probe-at 62` と無音対照を走らせて比べる手順を渡してある。
...
**Phase 4(可塑性)の候補。** 「感受性など 1〜2 個の係数を経験に応じて更新し、無音区間または共通刺激の後にも残る差と、変更された係数を記録」がロードマップの定義。順応との違いは、m が刺激と共に戻るのに対し、係数は戻らない(または非常に遅く戻る)こと。

1. **利得 g そのものを経験で更新する。** 例: `g += η·(m − m_target)` を遅く回す。よく鳴る環境では g が下がったまま、静かな環境では上がる。順応(m)と可塑性(g)が二重の時定数になる。
2. **sigma(成長の幅)を更新する。** 個体の「丈夫さ」が変わる。ただし Orbium の生存域が狭いので、境界を先に測る。
3. **mu は触らない**(Phase 2 の結論のまま)。

**判断してほしいこと:**

1. Phase 4 は 1(利得の更新)から始めてよいか。
2. 作品の時定数を 10 s 程度にするか、60 s のように長くするか。前者は曲の中の盛り上がりで慣れ、後者は曲全体の音量で慣れる。
3. 勾配は cos 型を既定にするか(継ぎ目で連続、向きの変化は穏やか)、線形を残すか。
4. Mac での実曲の比較結果(履歴あり/なしの応答)を教えてほしい。報告に追記する。
5. 履歴が格子に残らない(m という 1 つの数にある)ことを、作品の意図として受け入れるか。「身体に残る」ことを求めるなら、Phase 4 で係数を格子上の場にする(空間的な可塑性)という方向になる。
