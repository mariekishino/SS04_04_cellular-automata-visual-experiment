...
> **正誤表(2026-09-24、Phase 4 で発見):** 3.5 節の「10 秒パルス」は実際には **2 秒**だった(合成パルス源の不具合、コミット bb39c7b で修正)。履歴の実験(3.1〜3.4)は影響を受けない: H1/H2 は 0.5 秒パルス、H3 は周期 0 を明示していて正しく 18 秒、探針は 0.5 秒。10 秒で再実験した結果は 3.5 節の末尾に追記した。
...
**正誤表: 本当に 10 秒当てた再実験(`experiments/out/phase3_fix`)。** 線形勾配 0.2 × 10 秒は、向きを 95 → 100° と変えた後、刺激終了の直後に個体が**消滅**した(Phase 2 の正誤表と同じ)。cos 型 0.2 × 10 秒は生存し、向きは刺激中に 59〜66° を行き来した後、**刺激終了後 100 ステップで 68〜69°(基準)に戻った**。2 秒のときは 60° のまま戻らなかったので、「向きは戻らない」は cos 型の長い刺激では成り立たない。理由は未解明(cos 型は中央が強く端が弱いので、個体が中央を通過するたびに逆向きに押される可能性がある)。

![cos 型 10 秒](03_report/erratum_shape_cos_x_10s.png)
